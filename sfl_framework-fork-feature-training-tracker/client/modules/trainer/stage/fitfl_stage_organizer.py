...
class FitFLStageOrganizer(BaseStageOrganizer):
    ...
    async def _in_round(self):
        elapsed_time = time.time() - self.training_params["round_start_time"]
        expected_round_end_time = self.training_params["round_end_time"] - (
            elapsed_time * 1.3
        )

        self.trained_epochs = await self.in_round.train(
            self.model_trainer,
