...
class PruneFLImportance(MagnitudeImportance):
    def __init__(self, 
                 layer_time = None,
                 group_reduction:str="sum", 
                 normalizer:str='gaussian',
                 multivariable:bool=False, 
                 bias=False,
                 target_types:list=[nn.modules.conv._ConvNd, nn.Linear, nn.modules.batchnorm._BatchNorm, nn.modules.LayerNorm]):
    ...
    # At this point, PruneFL server already assigned the square of gradient
    @torch.no_grad()
    def __call__(self, group, ch_groups=1):
        group_imp = []
        group_idxs = []
        ...
        for i, (dep, idxs) in enumerate(group):
            ...
            layers = self.layer_time.keys()
            layer_name = layer_name.split(" ")[0]
            current_layer = list(filter(lambda x: layer_name in x, layers))[0]
            lt = self.layer_time[current_layer]
            # lt = 1
            ...
            if prune_fn in [
                ...
                if hasattr(layer, "transposed") and layer.transposed:
                    ...
                    dw = layer.weight.grad.data[idxs].flatten(1)
                if self.multivariable:
                    local_imp = (dw).sum(1).abs()
                else:
                    local_imp = (dw).abs().sum(1)
                group_imp.append(local_imp / lt)
                group_idxs.append(root_idxs)
