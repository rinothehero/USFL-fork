...
class EmaPruner:
    def __init__(
        ...
        pruning_ratio_dict: typing.Dict[
            nn.Module, float
        ] = None,  # layer-specific pruning ratio, will cover pruning_ratio if specified
        iterative_steps: int = 1,  # for iterative pruning
        iterative_pruning_ratio_scheduler: typing.Callable = scheduler.linear_scheduler,  # scheduler for iterative pruning.
        round_to: int = None,  # round channels to the nearest multiple of round_to
        # EMA
        ema_length: int = 3,  # exponential moving average length
        _ema: ema.EMA = None,  # EMA object. If None, a new EMA object will be created.
        ema_threshold=0.25,
        # Advanced
        in_channel_groups: typing.Dict[
            nn.Module, int
        ] = dict(),  # The number of channel groups for layer input
        out_channel_groups: typing.Dict[
            nn.Module, int
        ] = dict(),  # The number of channel groups for layer output
        num_heads: typing.Dict[
            nn.Module, int
        ] = dict(),  # The number of heads for multi-head attention
        ...
        head_pruning_ratio_dict: typing.Dict[
            nn.Module, float
        ] = None,  # layer-specific head pruning ratio
        customized_pruners: typing.Dict[
            typing.Any, function.BasePruningFunc
        ] = None,  # pruners for customized layers. E.g., {nn.Linear: my_linear_pruner}
        unwrapped_parameters: typing.Dict[
            nn.Parameter, int
        ] = None,  # unwrapped nn.Parameters & pruning_dims. For example, {ViT.pos_emb: 0}
        ...
        self.pruning_ratio_dict = (
            pruning_ratio_dict if pruning_ratio_dict is not None else {}
        )
        self.max_pruning_ratio = max_pruning_ratio
        self.global_pruning = global_pruning
            ...
            for module in pruning_ratio_dict:
                ...
                for submodule in module.modules():
                    ...
                    if isinstance(submodule, prunable_types):
                        self.pruning_ratio_dict[submodule] = (
                            self.iterative_pruning_ratio_scheduler(
        ...
        if head_pruning_ratio_dict is not None:
            for module in head_pruning_ratio_dict:
                ...
                for submodule in module.modules():
                    ...
                    if isinstance(submodule, prunable_types):
                        self.head_pruning_ratio_dict[submodule] = (
                            self.iterative_pruning_ratio_scheduler(
                                ratio, self.iterative_steps
                            )
                        )
        ...
        if self.global_pruning:
            ...
            for group in self.DG.get_all_groups(
                ...
                initial_total_channels += (
                    self.DG.get_out_channels(group[0][0].target.module)
                ) // self._get_channel_groups(group)
                for dep, _ in group:
                    if (
                        dep.target.module in self.num_heads
                        and self.DG.is_out_channel_pruning_fn(dep.handler)
                    ):
            ...
            print("[EMA Pruner] initial_total_channels: ", self.initial_total_channels)
            print("[EMA Pruner] initial_total_heads: ", self.initial_total_heads)
    ...
    def update_ignored_layers(self, model, ignored_layers) -> None:
        self.ignored_layers = []
        self.ignored_params = []
        ...
        self.ema.ignored_layers = self.ignored_layers

        self.DG = dependency.DependencyGraph().build_dependency(
            model,
            example_inputs=self.ema.example_inputs,
            forward_fn=None,
    ...
    def get_target_pruning_ratio(self, module) -> float:
        s = self.pruning_ratio_dict.get(module, self.per_step_pruning_ratio)[
            self.current_step
        ]
    ...
    def get_pruned_ratio(self) -> float:
        return self.pruned_channels / (self.initial_total_channels)
    ...
    def _check_pruning_ratio(self, group) -> bool:
        for dep, _ in group:
            ...
            if self.DG.is_out_channel_pruning_fn(pruning_fn):
                ...
                if (
                    layer_out_ch
                    < self.layer_init_out_ch[module] * (1 - self.max_pruning_ratio)
                    or layer_out_ch == 1
                ):
                    return False
                ...
                if (
                    layer_in_ch
                    < self.layer_init_in_ch[module] * (1 - self.max_pruning_ratio)
                    or layer_in_ch == 1
                ):
                    return False
        return True
    ...
    def _is_attn_group(self, group) -> bool:
        ...
        for dep, _ in group:
            ...
            pruning_fn = dep.handler
            if (
                self.DG.is_out_channel_pruning_fn(pruning_fn)
                and module in self.num_heads
            ):
    ...
    def _downstream_node_as_root_if_attention(self, group):
        ...
        for _dep, _idxs in group:
            if (
                _dep.source.module in self.num_heads
                and self.DG.is_out_channel_pruning_fn(_dep.handler)
            ):
                is_attention = True
            if isinstance(
                _dep.target.module, tuple(self.root_module_types)
            ) and self.DG.is_in_channel_pruning_fn(_dep.handler):
                downstream_dep = _dep
        if (
            is_attention and downstream_dep is not None
        ):  # use a downstream node as the root node for attention layers
            group = self.DG.get_pruning_group(
                downstream_dep.target.module, downstream_dep.handler, _idxs
            )
        return group
            ...
            if self._check_pruning_ratio(group):  # check pruning ratio
                ...
                if self.DG.is_out_channel_pruning_fn(pruning_fn):
                    ...
                    n_pruned = current_channels - int(
                        self.layer_init_out_ch[module] * (1 - target_pruning_ratio)
                    )
                else:
                    current_channels = self.DG.get_in_channels(module)
                    target_pruning_ratio = self.get_target_pruning_ratio(module)
                    n_pruned = current_channels - int(
                        self.layer_init_in_ch[module] * (1 - target_pruning_ratio)
                    )
                ...
                if n_pruned > 0:
                    if (self.prune_head_dims and _is_attn) or (not _is_attn):
                        ...
                        if self.round_to:
                            n_pruned_per_group = self._round_to(
                                n_pruned_per_group, group_size, self.round_to
                            )
                        if n_pruned_per_group > 0:
                            for chg in range(ch_groups):
                                sub_group_imp = imp[
                                    chg * group_size : (chg + 1) * group_size
                                ]
                                sub_imp_argsort = torch.argsort(sub_group_imp)
                                sub_pruning_idxs = (
                                    sub_imp_argsort[:n_pruned_per_group]
                                    + chg * group_size
                                )  # offset
                ...
                if _is_attn and self.prune_num_heads:  # Prune entire attn heads
                    target_head_pruning_ratio = self.get_target_head_pruning_ratio(
                        qkv_layers[0]
                    )
                    n_heads_removed = self.num_heads[qkv_layers[0]] - int(
                        self.init_num_heads[qkv_layers[0]]
                ...
                ema_threshold = self.ema.get_ema_threshold(group, self.ema_threshold)
                print(f"EMA threshold: {ema_threshold}")

                ema_mask = self.ema.get_ema_mask(group, pruning_idxs, ema_threshold)
                masked_pruning_indices = [
                    idx for idx, mask in zip(pruning_idxs, ema_mask) if mask
                ]

                if len(masked_pruning_indices) <= 0:
                    continue

                group = self.DG.get_pruning_group(
                    module, pruning_fn, masked_pruning_indices
                )
                ...
                print(f"Skipped group: {group}")

    def prune_global(self) -> typing.Generator:
        print("run prune global")

        pruned_ratio = self.get_pruned_ratio()
        prunable_ratio = (
            0
            if self.pruning_ratio - pruned_ratio <= 0
            else self.pruning_ratio - pruned_ratio
        )
        if prunable_ratio <= 0:
            warnings.warn(
                "Pruning ratio is already reached, no pruning will be performed."
            )
            return

        print(f"prunable_ratio: {prunable_ratio}")
        # if self.current_step > self.iterative_steps:
        ...
        for group in self.DG.get_all_groups(
            ...
            if self._check_pruning_ratio(group):
                ...
                if (
                    _is_attn
                    and self.prune_num_heads
                    and self.get_target_head_pruning_ratio(qkv_layers[0]) > 0
                ):
                    ...
                    head_imp = imp.view(ch_groups, -1).mean(
                        1
        ...
        if len(global_importance) > 0:
            concat_imp = torch.cat(
                [local_imp[-1] for local_imp in global_importance], dim=0
            )
            # target_pruning_ratio = self.per_step_pruning_ratio[self.current_step]
            target_pruning_ratio = prunable_ratio
            n_pruned = len(concat_imp) - int(
                (self.initial_total_channels - self.pruned_channels)
        ...
        if len(global_head_importance) > 0:
            concat_head_imp = torch.cat(
                [local_imp[-1] for local_imp in global_head_importance.values()], dim=0
            )
            target_head_pruning_ratio = self.per_step_head_pruning_ratio[
                self.current_step
            ]
            n_heads_removed = len(concat_head_imp) - int(
                self.initial_total_heads * (1 - target_head_pruning_ratio)
            )
            if n_heads_removed > 0:
        ...
        for group, ch_groups, group_size, imp in global_importance:
            ...
            if len(global_importance) > 0 and n_pruned > 0:
                if (
                    ch_groups > 1
                ):  # re-compute importance for each channel group if channel grouping is enabled
                    ...
                    if n_pruned_per_group > 0:
                        ...
                        if not _is_attn or self.prune_head_dims == True:
                            raw_imp = self.estimate_importance(
                                group, ch_groups=ch_groups
                            )  # re-compute importance
                            for chg in range(
                                ch_groups
                            ):  # determine pruning indices for each channel group independently
                                sub_group_imp = raw_imp[
                                    chg * group_size : (chg + 1) * group_size
                                ]
                                sub_imp_argsort = torch.argsort(sub_group_imp)
                                sub_pruning_idxs = (
            ...
            ema_mask = self.ema.get_ema_mask(group, pruning_indices, ema_threshold)
            masked_pruning_indices = [
                idx for idx, mask in zip(pruning_indices, ema_mask) if mask
            ]

            if len(masked_pruning_indices) <= 0:
                continue

            self.pruned_channels += len(masked_pruning_indices)
