...
class PruneFLPruner:
    """
        PruneFLPruner for structural pruning. 
    ...
    def __init__(
        ...
        # prunefl
        layer_times: typing.List[str] = None,
    ...
    def step(self, interactive=False)-> typing.Union[typing.Generator, None]:
        ...
        if len(pruned_groups)>0:
            self.pruned_groups = pruned_groups
        
    def restore(self) -> None:
        if self.pruned_groups == None:
            warnings.warn("No previous pruning history found. No restoration will be performed.")
            return
        
        for group in self.pruned_groups:
            group.restore()
    ...
    def prune_global(self) -> typing.Generator:
        ...
        if len(global_importance)>0:
            ...
            if n_pruned>0:
                ...
                # t = np.mean(self.importance.layer_time.values())
                # delta = 0
                
                # for imp in concat_imp:
                #     if imp >= delta/t or n_grown <= n_required:
                #         delta += imp
                #         t += 
                #     else:
                #         thres = imp
